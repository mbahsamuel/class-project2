        ...
        stage('AWS STS Stage') {
            steps {
                script {
                    ...
                    sh "aws cloudformation deploy --template-file ${TEMPLATE_FILE} --stack-name ${STACK_NAME} --capabilities CAPABILITY_IAM --region ${AWS_REGION} --s3-bucket desamist001 --s3-prefix cf-templates/v77"
